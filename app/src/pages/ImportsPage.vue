        ...
        <div style="width: 100%;text-align: center;">
          ...
          <div class="row">
            ...
            <div class="col-lg-12">
              ...
              <div style="text-align: left;" class="alert alert-light" v-if="selects.broker === 'fidelity'">
                For Fidelity Transactions, Click Accounts and Trade in the top left navbar.  Then click Activities & Orders in the white navbar in the center of the page.  Select the timeframe you want , then click the download arrow on the right hand side.
              </div>
...
import {csvToDF, extractPeriodSlice,isClosingTrade} from "@/DataTransformer";
    ...
    readFile() {
      if (!this.selects.broker) {
        ...
        reader.onload = async (res) => {
          ...
          const startDate = this.$store.state.startDate;
          const endDate = this.$store.state.endDate;
          const period_df = extractPeriodSlice(df, startDate, endDate);
          ...
          const periodSize = JSON.stringify(period_df.data).replace(/[\[\]\,\"]/g, '').length; //stringify and remove all "stringification" extra data
          ...
          this.message = 'You imported ' + df.length + ' rows to local storage with ' + closers.length + ' closes.';
          this.message_class = 'alert alert-success';
