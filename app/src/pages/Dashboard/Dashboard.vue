  ...
  <div class="row">
    ...
    <div class="col-12" style="width: 100%;">
      <div v-if="is_first_time" class="alert alert-gray" style="color: black;">
        StJou runs entirely in <span style="text-decoration: underline;color: black;">in your browser, <b>on your machine.</b></span>
        So your data never leaves your computer. <a href="https://github.com/charlie-sanders/stjou"
                                                                        target="_blank"
                                                                        style="color: whitesmoke;text-decoration: underline;">Click
          ...
          <vc-date-picker v-model="range" mode="date" :masks="masks" is-range>
            <template v-slot="{ inputValue, inputEvents, isDragging }">
              ...
              <div class="">
                ...
                &nbsp;
                <el-select class="select-info" size="large" placeholder="Ticker" v-model="filterSymbol">
                  <el-option v-for="sym in dashboard.selects.syms" class="select-info" :value="sym" :label="sym"
                             :key="sym"></el-option>
                </el-select> &nbsp; <a v-if="filterSymbol" href="javascript:;" @click="resetSymbols">Reset</a> &nbsp;
    ...
    <div class="col-lg-3 col-md-6">
      <stats-card :title="toCurrency(dashboard.averagePerTrade)" sub-title="Average Per Trade" type="dollar"
        ...
        <div slot="footer">
          {{ dashboard.numberOfTrades }} total closed ( {{ dashboard.numberOfWins }} wins, {{
            dashboard.numberOfLosses
          }} losses)
        </div>
      </stats-card>
    </div>
      ...
      <card type="chart">
        ...
        <div class="chart-area">
          <weekly-line-chart :chart-data="chart_line_last_5_grouped.chartData"
                             :chart-options="chart_line_last_5_grouped.options" :on-click="onClickChartLast5Grouped"
...
import {extractPeriodSlice, generateTradingData, isClosingTrade, toCurrency, toDisplayDate} from 'src/DataTransformer'
...
import {Button, MessageBox, Option, Select} from "element-ui";
...
import {ensureDataOffsetIndex, randomString, subtractDateStrings} from "@/util";
...
function _onClickChartWinLoss(evt, activeElements, chart, skipOffset = false) {
  ...
  if (points.length) {
    ...
    winning_symbols += `<tr><td width="200px">Total</td><td colspan="2" width="200px" style="font-size: 1.2em;"><i style="font-size: .8em;" class="fas fa-thin fa-arrow-up stj-green-text"></i>  ${toCurrency(total_win)}</td></tr>`
...
export default {
  ...
  data() {
    return {
      ...
      closers: null,
      filterSymbol: this.$store.state.filterSymbol,
  ...
  computed: {
    dashboard() {
      let filterSymbol = this.filterSymbol;
      ...
      if (this.filterSymbol) {
        // alert('HERE FUCKHEAD!')
        console.log('FILTER SYMBOL', this.filterSymbol)
        periodDF = periodDF.filter(row => row.symbol === this.filterSymbol);
      }
      ...
      const syms = [...new Set(this.closers.map(row => row.symbol).data)]
  ...
  methods: {
    ...
    onClickChartLast5Grouped: (evt, activeElements, chart) => {
      const points = chart.getElementsAtEventForMode(evt, 'nearest', {intersect: true}, true);
      const id = chart.data.datasets[0].id;
      console.log(`ID: ${id}`);
      if (points.length) {
        ...
        const offset = closersGrouped.data.length - Math.abs(5 - index);
        console.log('offset', offset)
        const row = closersGrouped.data[offset];
        console.log('row', row)
...
`
  ...
  watch: {
    ...
    filterSymbol: function (newVal, oldVal) {
      if (newVal !== oldVal) {
        this.$store.commit('setFilterSymbol', newVal);
        // this.closers = closers.filter(row => row.symbol === newVal);
      }
    },
  },
  mounted() {
