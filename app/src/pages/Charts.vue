...
<!--                  <button type="button" @click="resetSymbols()" class="btn btn-grey btn-round btn-outline">-->
<!--                    Reset-->
<!--                  </button>-->
...
export default {
  ...
  computed: {
    dashboard() {
      ...
      if (periodDF) {
        let closers = periodDF.filter(row => isClosingTrade(row));
        if ( this.filterSymbol )
        {
          // alert('HERE FUCKHEAD!')
          console.log('FILTER SYMBOL', this.filterSymbol)
          closers = closers.filter(row => row.symbol === this.filterSymbol);
        }
        // closers['days_held'] =
        const days_held = [];
        const sell_month = [];
        ...
        // if ( this.filterSymbol)
        // {
        //   console.log('FILTER SYMBOL IS PRESENT: ', this.filterSymbol)
        //   this.closers = closers.filter(row => row.symbol === this.filterSymbol);
        // }
        // else {
        //   console.log('FILTER SYMBOL IS NOT PRESENT: ', this.filterSymbol)
        // }
