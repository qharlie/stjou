    ...
    sell(symbol, qty, price, date, stock_or_option, short_or_long, estimate_missing_values) {
        ...
        if (short_or_long == 'short') {
            if (symbol in this.priceTable && this.priceTable[symbol].length > 0) {
                ...
                // } else
                // {
                //     console.log(symbol, this.priceTable[symbol],qty, price, date, stock_or_option, short_or_long, estimate_missing_values);
                //     throw new Error('Cant sell short a stock you already are long on');
                // }
            }
            return this.buy(symbol, qty, price, date, stock_or_option, short_or_long, estimate_missing_values);
        } else {
        ...
        if (row[KeyTable.DESCRIPTION]) {
            ...
            if (row && row[KeyTable.DESCRIPTION] && row[KeyTable.DESCRIPTION].startsWith('Bought')) {
                ...
                if (pnl_object == -1) {
                    ...
                    new_row[TradeSpec.effective_sell_price] = new_row[KeyTable.SELL_PRICE]
                    new_row[TradeSpec.effective_buy_price] = new_row[KeyTable.BUY_PRICE]
...
function standardizeFidelity(rows) {
    return rows;
}
...
export function standardizeBrokers(rows, broker) {
    if (broker === Brokers.ameritrade) {
        ...
        return standardizeFidelity(rows);
...
function transformEtradeToStandardFormat(json) {
...
function transformFidelityToStandardFormat(json) {
    ...
    for (let i = 0; i < json.length; i++) {
        ...
        const new_row = {};
        if ('Action' in row) {
            // If Description contains the strings BOUGHT or SOLD, then add it to new_rows
            if (row['Action'].includes('YOU BOUGHT') || row['Action'].includes('YOU SOLD')) {
                ...
                new_row[KeyTable.DATE] = row['Settlement Date'].trim();
                new_row[KeyTable.SYMBOL] = row['Symbol'].trim();
                new_row[KeyTable.DESCRIPTION] = row['Action'].trim();
                // Replace the the string 'YOU ' from the beggining of the description
                new_row[KeyTable.DESCRIPTION] = new_row[KeyTable.DESCRIPTION].replace('YOU ', '').trim();
                // Replace the word 'SOLD' with 'Sold' and 'BOUGHT' with 'Bought'
                new_row[KeyTable.DESCRIPTION] = new_row[KeyTable.DESCRIPTION].replace('SOLD', 'Sold').trim();
                new_row[KeyTable.DESCRIPTION] = new_row[KeyTable.DESCRIPTION].replace('BOUGHT', 'Bought').trim();

                new_row[KeyTable.QUANTITY] = Math.abs(safeParseInt(row['Quantity']));
                new_row[KeyTable.PRICE] = safeParseFloat(row[priceKey]);
                new_row[KeyTable.AMOUNT] = safeParseFloat(row[amountKey]);
                new_row[KeyTable.FEES] = safeParseFloat(row[feeKey]);
                new_row[KeyTable.TRANSACTION_ID] = randomString(10);
...
function cleanupCSVString(csv_string, broker) {
    if (broker === Brokers.etrade) {
        ...
        // We expect the first column name to be Run Date, so delete anything before that
        let index = csv_string.indexOf('\n\nRun Date');
        if (index > 0) {
            csv_string = csv_string.substring(index+2);
        }

        // Now remove anything after two new lines in a row
        index = csv_string.indexOf('\n\n');
        if (index > 0) {
            csv_string = csv_string.substring(0, index);
        }
        return csv_string;
...
export async function csvToDF(csv_string, broker = 'ameritrade', estimateMissingValues = false, strict_checking = true) {
    ...
    const json = await csv().fromString(cleanupCSVString(csv_string, broker));
    // console.log('json', json)
    let cleaned_json = {};
    let new_rows = [];
    if (broker === Brokers.etrade) {
        // console.log('Now transforming etrade csv to standard format', json);
        cleaned_json = transformEtradeToStandardFormat(json);
        ...
        cleaned_json = transformFidelityToStandardFormat(json);
    ...
    new_rows = addDataPoints(cleaned_json, estimateMissingValues, strict_checking);
